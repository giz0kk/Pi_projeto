(function () {
  ...
  function TransportButton({ item, data }) {
    ...
    const tooltip = `${item.label}: ${data.time}${data.detail ? ' · ' + data.detail : ''}`;

    return e('div', { className: 'group relative flex flex-col items-center' },
      e('button', {
        type: 'button',
        className: 'flex h-11 w-11 items-center justify-center rounded-2xl border border-emerald-900/10 bg-white/85 text-emerald-900 shadow-sm backdrop-blur transition duration-200 ease-out hover:-translate-y-1 hover:border-emerald-500/40 hover:bg-emerald-50 hover:text-emerald-700 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-emerald-500/40 sm:h-12 sm:w-12',
        'aria-label': tooltip,
      }, e(Icon, { className: 'h-5 w-5 sm:h-6 sm:w-6' })),
      e('span', {
        role: 'tooltip',
        className: 'pointer-events-none absolute top-full z-[900] mt-2 w-max max-w-[190px] translate-y-1 rounded-xl border border-emerald-900/10 bg-slate-950/90 px-3 py-2 text-center text-[11px] font-semibold leading-snug text-white opacity-0 shadow-xl backdrop-blur transition duration-200 group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:translate-y-0 group-focus-within:opacity-100',
      }, tooltip),
    );
  }
  ...
  function TransportTimesWidget() {
    ...
    if (!times) {
      return e('div', { className: 'flex items-center gap-2 text-xs font-semibold text-slate-500' },
        e('span', { className: 'h-2 w-2 animate-pulse rounded-full bg-emerald-400' }),
        'Calculando tempos...'
      );
    }

    return e('div', { className: 'font-sora flex w-full items-center gap-3' },
      e('div', { className: 'hidden min-w-0 flex-col pr-1 md:flex' },
        e('span', { className: 'text-[10px] font-bold uppercase tracking-[0.16em] text-emerald-700' }, 'Tempo'),
        e('span', { className: 'text-xs font-semibold text-slate-500' }, 'por transporte')
      ),
      e('div', { className: 'grid flex-1 grid-cols-4 gap-2 sm:gap-3' },
        TRANSPORTS.map((item) => e(TransportButton, { key: item.key, item, data: times[item.key] }))
      )
    );
  }
