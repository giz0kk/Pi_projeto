  ...
  if (cardDetalhado) {
    ...
    cards.forEach(card => {
      card.addEventListener("click", () => {
        ...
        if (existing && existing.classList.contains("card-ecoponto")) {
          existing.remove();
        } else {
          // hide any other
          document.querySelectorAll(".card-ecoponto").forEach(d => d.remove());
          card.insertAdjacentElement("afterend", cardDetalhado);
