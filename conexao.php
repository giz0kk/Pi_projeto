 ...
 *
 * Alternativa: criar um arquivo conexao.config.php na raiz do projeto retornando:
 *   <?php return [
 *     "host" => "localhost",
...
if ($conn->connect_errno === 1049 && $autoInstall) {
    ...
    if (!ecocoleta_criar_banco_e_importar($boot, $banco, __DIR__)) {
        ecocoleta_falhar_conexao(
...
if ($conn->connect_errno && $host === "localhost") {
    ...
    if ($conn2->connect_errno === 1049 && $autoInstall) {
        ...
        if (!$boot->connect_errno) {
            ecocoleta_criar_banco_e_importar($boot, $banco, __DIR__);
            @$boot->close();
            $conn2 = ecocoleta_tentar_conectar("127.0.0.1", $usuario, $senha, $banco);
        }
    }
...
if ($precisaMigrar && $autoInstall) {
    ecocoleta_criar_banco_e_importar($conn, $banco, __DIR__);
}
